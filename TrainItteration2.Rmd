...
### Done by: Behzad
```{r}
dt3 <- df <- read_csv("./trigrams.csv", col_types = cols(cyl = col_number()),na = "NA")
```
#### Density Plot

```{r warning=FALSE,message=FALSE}
energy.related.repos<-as.numeric(unlist(subset(dt3,target==1,)))
not.energy.related.repos<-as.numeric(unlist(subset(dt3,target==0,)))
...
#### Scatter Plot
```{r warning=FALSE,message=FALSE}
# energy.related.repos<-subset(dt2,target==1,)
# not.energy.related.repos<-subset(dt2,target==0,)
# ggplot(NULL, aes(x=dt2$, y=target)) +
#   geom_point(data=energy.related.repos, aes(x=score, y=target,color="Energy Related"))+
#   geom_point(data=not.energy.related.repos, aes(x=score, y=target,color="Not Energy Related"))
```
#### K-Fold
